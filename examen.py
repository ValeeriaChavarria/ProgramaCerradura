...
def controlar_primera_cerradura(A: bool, B: bool, C: bool, D: bool) -> bool:
    """Control lógico de la primera cerradura
       ...
       - D es verdadero y A, B y C son verdaderos.
    """
    A_y_B = y(A, B)
    C_negado = neg(C)
    C_negado_y_A_y_B = y(C_negado, A_y_B)
    D_y_A_y_B_y_C = y(D, A_y_B and C)

    return o(A_y_B, o(C_negado_y_A_y_B, D_y_A_y_B_y_C))
...
def main() -> int:
    ...
    global D1  # Hacer D1 una variable global
    foo()
    print(f"Hora actual: {horaActual()}")

    # Control de la primera cerradura
    estado_primera_cerradura = controlar_primera_cerradura(A1, B1, C1, D1)
